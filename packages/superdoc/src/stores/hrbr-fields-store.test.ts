...
vi.mock('./superdoc-store.js', () => {
  ...
  const activeSelection = reactive({ documentId: null as string | null, selectionBounds: {} });
...
function componentStub(name: string) {
  return { default: { name } };
}
...
describe('hrbr-fields-store', () => {
  let store: ReturnType<typeof useHrbrFieldsStore>;
    ...
    __mockSuperdoc.documents.value = [
      {
        id: 'doc-1',
        fields: [
          { itemid: 'field-1', label: 'Field One' },
          { itemid: 'field-2', label: 'Field Two' },
        ],
      },
    ];

    expect(store.getField('doc-1', 'field-2')).toEqual({ itemid: 'field-2', label: 'Field Two' });
  ...
  it('maps annotations to positioned fields', () => {
    ...
    const getElementSpy = vi.spyOn(document, 'getElementById').mockReturnValue(pageElement as HTMLElement);

    __mockSuperdoc.documents.value = [
      {
        id: 'doc-1',
        fields: [{ itemid: 'field-1' }],
        annotations: [
          {
            itemid: 'field-1',
            page: 0,
