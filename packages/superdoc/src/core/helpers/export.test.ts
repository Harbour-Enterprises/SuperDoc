...
describe('createDownload', () => {
  ...
  beforeEach(() => {
    ...
    document.createElement = vi.fn(() => ({ href: '', download: '', click: clickMock }) as unknown as HTMLElement);
  ...
  it('converts ArrayBuffer-like inputs into a Blob before downloading', () => {
    ...
    const createdBlob = (URL.createObjectURL as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(createdBlob).toBeInstanceOf(Blob);
    expect((createdBlob as Blob).size).toBe(3);
    expect((createdBlob as Blob).type).toBe('application/vnd.openxmlformats-officedocument.wordprocessingml.document');
  ...
  it('wraps string input with the correct mime type', () => {
    ...
    const createdBlob = (URL.createObjectURL as ReturnType<typeof vi.fn>).mock.calls[0][0] as Blob;
  ...
  it('throws for nullish inputs', () => {
    expect(() => createDownload(null as unknown as Blob, 'invalid', 'docx')).toThrow(TypeError);
