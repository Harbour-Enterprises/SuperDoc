 ...
 * Storage for Search
 */
export interface SearchStorage extends Record<string, unknown> {
 ...
 */
export interface SearchCommandOptions extends Record<string, unknown> {
  /**
...
export const Search = Extension.create<SearchOptions, SearchStorage>({
  ...
  addCommands(): unknown {
    return {
       ...
       * @category Command
       * @param patternInput - Search string or pattern
       * @param options - Options to control search behavior
       * @example
      ...
      search:
        ...
        ({ state, dispatch }: { state: EditorState; dispatch?: (tr: Transaction) => void }): SearchMatch[] => {
          ...
          const resultMatches: SearchMatch[] = matches.map((d) => ({
            from: d.from,
            to: d.to,
       ...
       * @category Command
       * @param match - Match object to navigate to
       * @example
