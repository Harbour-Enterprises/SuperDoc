  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, oldState, _, newState) {
        ...
        if (selection instanceof NodeSelection && nodeNames.includes(selection.node?.type.name)) {
          decorations.push(
            Decoration.node(selection.from, selection.to, {
              nodeName: 'span',
              class: 'sd-editor-resizable-wrapper',
              'data-pos': String(selection.from),
            }),
          );
        }
