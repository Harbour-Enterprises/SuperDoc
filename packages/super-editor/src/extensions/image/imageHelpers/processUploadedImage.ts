 ...
 * @category Helper
 * @param fileData - Base64 string or File object
 * @param getMaxContentSize - Function returning max width/height constraints
...
): Promise<string | { file: File; width: number; height: number }> => {
 ...
 */
export const getAllowedImageDimensions = (
  width: number,
  height: number,
  getMaxContentSize: () => { width: number; height: number },
 ...
 * @param resize_canvas - Whether to resize the canvas element
 */
function resample_high_quality(canvas: HTMLCanvasElement, width: number, height: number, resize_canvas: boolean): void {
 ...
 * @param height - Target height
 */
function multiStepResize(canvas: HTMLCanvasElement, width: number, height: number): void {
