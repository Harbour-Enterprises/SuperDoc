...
export const BlockNode = Extension.create({
  ...
  addHelpers() {
    return {
       ...
       */
      getBlockNodes: (): Array<{ node: ProseMirrorNode; pos: number }> => {
        return findChildren((this.editor as { state: { doc: ProseMirrorNode } }).state.doc, (node: ProseMirrorNode) =>
          nodeAllowsSdBlockIdAttr(node),
        );
      },
       ...
       */
      getBlockNodeById: (id: string): Array<{ node: ProseMirrorNode; pos: number }> => {
        return findChildren(
          (this.editor as { state: { doc: ProseMirrorNode } }).state.doc,
          (node: ProseMirrorNode) => node.attrs.sdBlockId === id,
        );
      },
       ...
       */
      getBlockNodesByType: (type: string): Array<{ node: ProseMirrorNode; pos: number }> => {
        return findChildren(
          (this.editor as { state: { doc: ProseMirrorNode } }).state.doc,
          (node: ProseMirrorNode) => node.type.name === type,
        );
      },
       ...
       * )
       */
      getBlockNodesInRange: (from: number, to: number) => {
        const blockNodes: Array<{ node: ProseMirrorNode; pos: number }> = [];

        (this.editor as { state: { doc: ProseMirrorNode } }).state.doc.nodesBetween(
          from,
          to,
  ...
  addPmPlugins() {
     ...
     */
    const assignBlockId = (tr: Transaction, node: ProseMirrorNode, pos: number) => {
      tr.setNodeMarkup(
        pos,
        undefined,
        {
      ...
      new Plugin({
        ...
        appendTransaction: (transactions: readonly Transaction[], oldState: EditorState, newState: EditorState) => {
          ...
          if (!hasInitialized) {
            ...
            const rangesToCheck: Array<[number, number]> = [];
 ...
 */
export const nodeAllowsSdBlockIdAttr = (node: ProseMirrorNode): boolean => {
 ...
 */
export const nodeNeedsSdBlockId = (node: ProseMirrorNode): boolean => {
 ...
 */
export const checkForNewBlockNodesInTrs = (transactions: readonly Transaction[]): boolean => {
