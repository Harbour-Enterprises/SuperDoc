...
export const cleanUpParagraphWithAnnotations =
  ...
  ({ dispatch, tr, state }: { dispatch?: (tr: Transaction) => void; tr: Transaction; state: EditorState }) => {
    ...
    const toDelete = new Map<number, boolean>();

    const sizeOf = (doc: PmNode): number => doc.content.size;
    const inRange = (doc: PmNode, pos: number): boolean => Number.isInteger(pos) && pos >= 0 && pos <= sizeOf(doc);
