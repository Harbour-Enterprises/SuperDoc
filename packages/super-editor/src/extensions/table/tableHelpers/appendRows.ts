...
type TableRowNode = PmNode & { attrs: Record<string, unknown>; content: { content: PmNode[] } };
type TableNode = PmNode & { attrs: Record<string, unknown> };
type RowTemplateFormatting = {
  blockType: NodeType;
  blockAttrs: Record<string, unknown> | null;
  textMarks: Mark[];
};
type BuildRowFromTemplateRowParams = {
  schema: Schema;
  tableNode: TableNode;
  templateRow: TableRowNode;
  values: unknown[];
  copyRowStyle?: boolean;
};
type InsertRowsAtTableEndParams = {
  tr: Transaction;
  tablePos: number;
  tableNode: TableNode;
  rows: PmNode[];
 ...
 */
export function resolveTable(tr: Transaction, tablePos?: number, tableNode?: PmNode | null) {
 ...
 */
export function pickTemplateRowForAppend(tableNode: TableNode, schema: Schema): TableRowNode | null {
  const RowType = schema.nodes.tableRow;
  const rows: TableRowNode[] = [];
  tableNode.descendants((child) => {
    if (child.type === RowType) rows.push(child as TableRowNode);
  });
 ...
 */
export function extractRowTemplateFormatting(cellNode: PmNode, schema: Schema): RowTemplateFormatting {
  const ParagraphType = schema.nodes.paragraph;
  let blockType: NodeType = ParagraphType;
  let blockAttrs: Record<string, unknown> | null = null;
  let textMarks: Mark[] = [];
  ...
  for (const block of blocks) {
    ...
    let foundText: PmNode | null = null;
    block.descendants?.((n) => {
      if (!foundText && n.isText) foundText = n as PmNode;
    });
    if (foundText) {
      const marks = (foundText as PmNode & { marks?: Mark[] }).marks;
      textMarks = marks ? Array.from(marks) : [];
      break;
    }
  }
 ...
 */
export function buildFormattedCellBlock(
  schema: Schema,
  value: unknown,
  { blockType, blockAttrs, textMarks }: RowTemplateFormatting,
  copyRowStyle = false,
) {
 ...
 */
export function buildRowFromTemplateRow({
  schema,
  tableNode,
  templateRow,
  values,
  copyRowStyle = false,
}: BuildRowFromTemplateRowParams) {
  const RowType = schema.nodes.tableRow;
  ...
  templateRow.content.content.forEach((cellNode, cellIndex) => {
    ...
    if (byColumns) {
      ...
      cellValue = values[columnCursor] == null ? '' : String(values[columnCursor]);
      columnCursor += span;
    } else {
      const value = Array.isArray(values) ? values[cellIndex] : '';
 ...
 */
export function insertRowsAtTableEnd({ tr, tablePos, tableNode, rows }: InsertRowsAtTableEndParams) {
  if (!rows || !rows.length) return;
  const RowTypeName = 'tableRow';
  let lastRowRelPos = 0;
  let lastRowNode: PmNode | null = null;
  ...
  const lastRowAbsEnd = tablePos + 1 + lastRowRelPos + (lastRowNode as PmNode).nodeSize;
