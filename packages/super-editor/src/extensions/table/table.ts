...
interface AppendRowsWithContentOptions {
  tablePos?: number;
  tableNode?: PmNode;
  valueRows?: string[][];
    ...
    return {
        ...
        ({ state, dispatch, chain }: CommandProps) => {
          ...
          return chain()
            ...
            .command(({ tr }: CommandProps) => {
              ...
              const newColumnIndex = rect.left;
              ...
              for (let row = 0; row < updatedMap.height; row++) {
                const cellIndex = row * updatedMap.width + newColumnIndex;
                const cellPos = updatedMap.map[cellIndex];
                const cellAbsolutePos = rect.tableStart + cellPos;
                const cell = tr.doc.nodeAt(cellAbsolutePos);
                if (cell) {
                  const attrs = {
                    ...currentCellAttrs,
        ...
        ({ state, dispatch, chain }: CommandProps) => {
          ...
          return chain()
            ...
            .command(({ tr }: CommandProps) => {
              ...
              if (!table) return false;
              const updatedMap = TableMap.get(table);
              const newColumnIndex = rect.left + 1;
          ...
          return chain()
            ...
            .command(({ tr }: CommandProps) => {
              ...
              for (let col = 0; col < updatedMap.width; col++) {
                const cellIndex = newRowIndex * updatedMap.width + col;
                const cellPos = updatedMap.map[cellIndex];
                const cellAbsolutePos = rect.tableStart + cellPos;
                const cell = tr.doc.nodeAt(cellAbsolutePos);
                if (cell) {
      ...
      splitSingleCell:
        ...
        ({ state, dispatch, tr }: CommandProps) => {
          ...
          if (dispatch) {
            ...
            const currentColWidth = baseAttrs.colwidth;
            ...
            for (let row = 0; row < rect.map.height; row++) {
              ...
              [...rowCells].forEach((cellIndex) => {
                ...
                if (cellRect.left <= currentCol && cellRect.right > currentCol) {
                  const cellPos = tr.mapping.map(rect.tableStart + cellIndex, 1);
                  const cell = tr.doc.nodeAt(cellPos);

                  if (cell) {
                    const newColspan = (cell.attrs.colspan || 1) + 1;
                    let updatedColwidth = cell.attrs.colwidth;
                    if (updatedColwidth && newCellWidth) {
                      const originalColIndex = currentCol - cellRect.left;
                      updatedColwidth = [
                    ...
                    const cellAttrs = { ...cell.attrs, colspan: newColspan, colwidth: updatedColwidth };
      ...
      setCellBackground:
        ...
        ({ editor, commands, dispatch }: CommandProps): boolean => {
          ...
          const color = typeof value === 'string' && value.startsWith('#') ? value.slice(1) : value;
