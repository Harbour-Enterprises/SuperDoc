...
type TrackChange = { mark: PmMark; from: number; to: number };
...
export const TrackChangesBasePlugin = () => {
  return new Plugin({
    ...
    state: {
      init(_, state): TrackChangesPluginState {
      ...
      apply(tr, oldState: TrackChangesPluginState, _prevEditorState, newEditorState): TrackChangesPluginState {
...
const getTrackChangesDecorations = (
  state: EditorState,
  onlyOriginalShown: boolean,
  onlyModifiedShown: boolean,
): DecorationSet => {
  ...
  const trackedChanges = getTrackChanges(state) as TrackChange[];
