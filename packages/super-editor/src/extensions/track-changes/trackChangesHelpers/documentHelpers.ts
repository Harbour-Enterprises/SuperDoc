...
type FoundMark = { from: number; to: number; attrs: Record<string, unknown> };

// https://discuss.prosemirror.net/t/expanding-the-selection-to-the-active-mark/
export const findMarkPosition = (doc: PmNode, pos: number, markName: string): FoundMark | null => {
...
export const flatten = (node: PmNode, descend: boolean = true): { node: PmNode; pos: number }[] => {
  if (!node) {
...
export const findInlineNodes = (node: PmNode, descend?: boolean): { node: PmNode; pos: number }[] => {
