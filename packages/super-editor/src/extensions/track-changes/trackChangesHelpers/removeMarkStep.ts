 ...
 */
export const removeMarkStep = ({ state, step, newTr, doc, user, date }: RemoveMarkStepParams): void => {
  const meta: { formatMark?: PmMark; step?: RemoveMarkStep } = {};
  ...
  doc.nodesBetween(step.from, step.to, (node: PmNode, pos: number) => {
    ...
    if (allowedMarks.includes(step.mark.type.name) && node.marks.find((mark) => mark.type === step.mark.type)) {
      ...
      if (formatChangeMark) {
        const attrs = formatChangeMark.attrs as { before: FormatChange[]; after: FormatChange[] };
        const foundAfter = attrs.after.find((mark) => mark.type === step.mark.type.name);

        if (foundAfter) {
          after = [...attrs.after.filter((mark) => mark.type !== step.mark.type.name)];
          before = [...attrs.before];
        } else {
          after = [...attrs.after];
          before = [
