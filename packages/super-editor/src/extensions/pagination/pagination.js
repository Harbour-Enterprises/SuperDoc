...
function createFloatingSeparator(separator, editor) {
  ...
  const { paginationFloating } = editor.options;
  if (paginationFloating?.className) {
    floatingSeparator.classList.add(paginationFloating.className);
  }
      ...
      middleware: [
        hide(),
        {
          ...
          fn: ({ elements }) => {
            ...
            const zoomFactor = getZoomFactor(editorElement);
            
            return {
              x: (rect.left - containerRect.left) / zoomFactor,
              y: (rect.top - containerRect.top) / zoomFactor,
              data: {
