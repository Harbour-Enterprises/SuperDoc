 ...
 */
export const createLinkedStylesPlugin = (editor: Editor): Plugin => {
  return new Plugin({
    ...
    state: {
      ...
      init() {
        ...
        const styles = (editor.converter?.linkedStyles as LinkedStyleDefinition[]) || [];
        return {
          styles,
      ...
      apply(
        tr: Transaction,
        prev: { styles?: LinkedStyleDefinition[]; decorations?: DecorationSet },
        oldEditorState: EditorState,
        newEditorState: EditorState,
      ): { styles?: LinkedStyleDefinition[]; decorations: DecorationSet } {
        if (!editor.converter || editor.options.mode !== 'docx')
          return { ...prev, decorations: prev.decorations || DecorationSet.empty };
        ...
        if (tr.docChanged) {
          ...
          tr.steps.forEach((step: Step, index: number) => {
            if (step instanceof ReplaceStep && (step as ReplaceStep).slice) {
              (step as ReplaceStep).slice.content.descendants((node: PmNode, pos: number) => {
            ...
            if (step instanceof AddMarkStep || step instanceof RemoveMarkStep) {
              ...
              if (markStep.mark && styleRelatedMarks.has(markStep.mark.type.name)) {
                mightAffectStyles = true;
              }
            }
 ...
 * @private
 */
const generateDecorations = (state: EditorState, styles: LinkedStyleDefinition[]): DecorationSet => {
  ...
  const getParagraphStyleId = (pos: number): string | null => {
  ...
  doc.descendants((node: PmNode, pos: number) => {
    ...
    let runStyleId: string | null = null;
    let inlineTextStyleId: string | null = null;
    for (const mark of node.marks) {
      if (mark.type.name === 'run') {
        ...
        if (rp && typeof rp === 'object' && !Array.isArray(rp) && rp.styleId) runStyleId = rp.styleId;
          ...
          const ent = rp.find((e: unknown) => (e as { xmlName?: string })?.xmlName === 'w:rStyle') as
    ...
    const buildStyleMap = (sid: string | null): Record<string, unknown> => {
      if (!sid) return {};
    ...
    let tMap: Record<string, unknown>;
