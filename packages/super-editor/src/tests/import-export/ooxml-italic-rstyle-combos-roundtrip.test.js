...
const collectExpectedFromSource = (doc) => {
  ...
  paragraphs.forEach((p) => {
    (p.elements || []).forEach((child) => {
      ...
      else italic = rStyle?.attributes?.['w:val'] === 'SD_ItalicStyle';
...
const collectItalicFromExport = (doc) => {
  ...
  paragraphs.forEach((p) => {
    (p.elements || []).forEach((child) => {
      if (child.name !== 'w:r') return;
      const rPr = find(child, 'w:rPr');
      const hasI = !!find(rPr, 'w:i');
      const textEl = find(child, 'w:t');
      const text = textEl?.elements?.find((e) => e.type === 'text')?.text;
      if (!text) return;
      runs.push({ text, italic: hasI });
    });
  });
  return runs;
};
...
describe('OOXML italic + rStyle combinations round-trip', async () => {
  ...
  const exportedRuns = collectItalicFromExport(exported);
