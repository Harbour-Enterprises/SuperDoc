 ...
 */
export const mergeRanges = (ranges: Array<[number, number]>): Array<[number, number]> => {
  if (ranges.length === 0) return [];

  // Sort ranges by start position and clone to avoid mutating callers
  const sorted = [...ranges].sort((a, b) => a[0] - b[0]).map(([start, end]): [number, number] => [start, end]);
 ...
 */
export const clampRange = (start: number, end: number, docSize: number): [number, number] | null => {
