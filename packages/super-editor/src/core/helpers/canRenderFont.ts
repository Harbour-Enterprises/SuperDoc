...
export function canRenderFont(fontName: string, fallbackFont = 'sans-serif'): boolean {
  const _canRenderFont = (fontNameToTest: string, fallbackFontName: string): boolean => {
    ...
    ctx.font = `72px ${fallbackFontName}`;
    ...
    ctx.font = `72px "${fontNameToTest}", ${fallbackFontName}`;
