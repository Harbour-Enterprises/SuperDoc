 ...
 */
const _createAttributeHandler = (xmlName) => {
  const sdName = xmlName.split(':')[1];
  return {
    xmlName,
    sdName,
...
const encode = (params, encodedAttrs) => {
  ...
  encodedAttrs['tableRowProperties'] = tableRowProperties;
...
const decode = (params, decodedAttrs) => {
  ...
  if (node.attrs?.tableRowProperties) {
    const trPr = trPrTranslator.decode(params);
