...
describe('w:trPr translator', () => {
  ...
  describe('encode', () => {
    it('encodes a complex <w:trPr> element correctly', () => {
      ...
      expect(result.attributes).toEqual({
        ...
        hidden: true,
        jc: 'center',
        tblCellSpacing: { value: 10, type: 'dxa' },
        repeatHeader: true,
  ...
  describe('round-trip', () => {
    it('maintains consistency for a complex object', () => {
      ...
      // The `tblCellSpacing.value` will be a string after encoding due to implementation.
      const expectedProperties = {
        ...tableRowProperties,
        tblCellSpacing: {
          ...tableRowProperties.tblCellSpacing,
          value: 10, // Becomes a string
        },
      };

      expect(encodedResult.attributes).toEqual(expectedProperties);
    });
  });
});
