...
export const preProcessNodesForFldChar = (nodes = [], docx) => {
   ...
   * @param {OpenXmlNode} node - The node that triggers the finalization (e.g., with fldCharType='end').
   */
  ...
  for (const node of nodes) {
    ...
    if (Array.isArray(node.elements)) {
      ...
      if (childResult.unpairedBegin) {
        ...
        childResult.unpairedBegin.forEach((pendingField, idx) => {
  ...
  if (collectedNodesStack.length > 0) {
    ...
    for (let i = 0; i < collectedNodesStack.length; i++) {
      processedNodes.push(...collectedNodesStack[i]);
      unpairedBegin.push({
 ...
 * @param {import('../v2/docxHelper').ParsedDocx} [docx] - The docx object.
 * @returns {OpenXmlNode[] | OpenXmlNode} The processed nodes.
 */
