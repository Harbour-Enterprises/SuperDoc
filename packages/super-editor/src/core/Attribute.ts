...
export class Attribute {
    ...
    return {
      ...
      getAttrs: (node: Element | string) => {
        if (typeof node === 'string') {
          return parseRule.getAttrs ? parseRule.getAttrs(node as any) : parseRule.attrs;
        }
        if (!Attribute.#isElementNode(node)) {
          return parseRule.getAttrs ? parseRule.getAttrs(node as any) : parseRule.attrs;
        }

        const elementNode = node;
        const oldAttrs = parseRule.getAttrs ? parseRule.getAttrs(elementNode as any) : parseRule.attrs;
        ...
        for (const item of extensionAttrs) {
          const value = item.attribute.parseDOM
            ? item.attribute.parseDOM(elementNode as HTMLElement)
            : parseFromString(elementNode.getAttribute(item.name));
   ...
   */
  static mergeAttributes(...objects: (Record<string, any> | null | undefined)[]): Record<string, any> {
    const items = objects.filter((item) => !!item) as Record<string, any>[];

    let attrs: Record<string, any> = {};

    for (const item of items) {
      const mergedAttributes: Record<string, any> = { ...attrs };
