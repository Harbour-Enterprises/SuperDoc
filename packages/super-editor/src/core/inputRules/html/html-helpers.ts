...
interface CreateSingleItemListParams {
  li: HTMLLIElement;
  rootNumId: string;
  level: number;
  listNumberingType: string;
...
interface BuildNestedListParams {
  items: ListItem[];
}
 ...
 */
export function flattenListsInHtml(html: string, editor: Editor): string {
  // pick the right parser & Node interface
  let parser: DOMParser;
  let NodeInterface: typeof Node;
  if (editor.options?.mockDocument) {
    const win = editor.options.mockDocument.defaultView as unknown as {
  ...
  const doc = removeWhitespaces(parser.parseFromString(html, 'text/html')) as Document;
 ...
 */
function findListToFlatten(doc: Document): HTMLElement | null {
  // First priority: unprocessed lists
  const list = doc.querySelector('ol:not([data-list-id]), ul:not([data-list-id])');
  ...
  for (const list of Array.from(allLists)) {
    const liChildren = Array.from(list.children).filter((c) => (c as Element).tagName.toLowerCase() === 'li');
 ...
 */
function flattenFoundList(listElem: HTMLElement, editor: Editor, _NodeInterface: typeof Node): void {
  ...
  if (!rootNumId) {
    rootNumId = String(ListHelpers.getNewListId(editor));
    ListHelpers.generateNewListDefinition({
      numId: parseInt(rootNumId, 10),
  ...
  const items = Array.from(listElem.children).filter((c) => c.tagName.toLowerCase() === 'li') as HTMLLIElement[];

  // Create single-item lists for each item
  const newLists: Array<HTMLElement | Element> = [];

  items.forEach((li) => {
    // Extract any nested lists first
    const nestedLists = Array.from(li.querySelectorAll('ol, ul'));
    const nestedListsData: NestedListData[] = nestedLists.map((nl) => ({
      element: nl.cloneNode(true) as Element,
    ...
    nestedListsData.forEach((data) => {
      ...
      data.element.setAttribute('data-level', String(level + 1));
 ...
 */
export function createSingleItemList({
  li,
  rootNumId,
  level,
  listNumberingType,
}: CreateSingleItemListParams): HTMLElement {
 ...
 */
export function unflattenListsInHtml(html: string): string {
  const parser = new DOMParser();
  const doc = parser.parseFromString(html, 'text/html');
  const allNodes = Array.from(doc.body.children);

  const listSequences: ListSequence[] = [];
  let currentSequence: ListSequence | null = null;

  allNodes.forEach((node) => {
    const element = node as HTMLElement;
    const isListParagraph = element.tagName === 'P' && element.hasAttribute('data-num-id');

    if (isListParagraph) {
      const listId = element.getAttribute('data-num-id');

      if (currentSequence && currentSequence.id === listId) {
        currentSequence.items.push(element);
      } else {
        currentSequence = {
          id: listId!,
          items: [element],
 ...
 */
function buildNestedList({ items }: BuildNestedListParams): HTMLElement | null {
  if (!items.length) {
    return null;
  }
  ...
  const lastLevelItem = new Map<number, HTMLLIElement>();
  items.forEach((item) => {
    const { element: paragraph, level } = item;
    const listItem = createListItemFromParagraph(paragraph);
    const cleanLi = cleanListItem(listItem) as HTMLLIElement;
    ...
    if (level === 0) {
      ...
      applyStartAttribute(nestedList as HTMLElement, item.listLevel, level);
...
function createListItemFromParagraph(paragraph: HTMLElement): HTMLLIElement {
...
function findNestedList(parentLi: HTMLLIElement, listTag: string): Element | undefined {
...
function getListTagForType(listNumberingType: string): string {
...
function applyStartAttribute(listNode: HTMLElement, listLevel: number[] | null, level: number): void {
...
function getStartValueForLevel(listLevel: number[] | null, level: number): number | null {
...
function parseListLevelAttribute(raw: string | null): number[] | null {
  if (!raw) {
    return null;
  }
 ...
 */
function cleanListItem(listItem: HTMLLIElement): HTMLLIElement {
  const attrs = [
    'data-num-id',
    'data-level',
    'data-num-fmt',
