...
export class PresentationEditor extends EventEmitter {
  ...
  #initHeaderFooterRegistry() {
    ...
    this.#headerFooterAdapter = new HeaderFooterLayoutAdapter(this.#headerFooterManager, mediaFiles);
  ...
  async #rerender() {
    ...
    try {
      ...
      const result = toFlowBlocks(docJson, {
        mediaFiles: this.#options.mediaFiles,
    ...
    if (this.#telemetryEmitter) {
      this.#telemetryEmitter({ type: 'layout', data: payload });
    }
  ...
  #computeHeaderFooterConstraints() {
    ...
    const width = pageSize.w - (margins.left + margins.right);
...
class PresentationInputBridge {
  ...
  #addListener(type: string, handler: (event: Event) => void, target: EventTarget) {
