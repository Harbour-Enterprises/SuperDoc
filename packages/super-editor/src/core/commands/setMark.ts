...
function canSetMark(editor: Editor, state: EditorState, tr: Transaction, newMarkType: MarkType): boolean {
  let { selection } = tr;
 ...
 */
//prettier-ignore
export const setMark = (typeOrName: string | MarkType, attributes: Record<string, unknown> = {}): Command => ({ tr, state, dispatch, editor }) => {
    let { selection } = tr;
    if (editor.options.isHeaderOrFooter && editor.options.lastSelection) {
      selection = editor.options.lastSelection as Selection;
    }
