...
export const toggleList =
  (listType: 'orderedList' | 'bulletList'): Command =>
  ({ editor, state, tr, dispatch }) => {
    ...
    let firstListNode: Node | null = null;
    let hasNonListParagraphs = false;
    const allParagraphsInSelection: Array<{ node: Node; pos: number }> = [];
    ...
    let mode: 'remove' | 'reuse' | 'create' | null = null;
    let sharedNumberingProperties: { numId: number; ilvl: number } | null = null;
    if (firstListNode) {
