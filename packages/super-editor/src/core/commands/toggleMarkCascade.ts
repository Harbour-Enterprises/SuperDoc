...
interface ToggleMarkCascadeOptions {
...
export const toggleMarkCascade =
  (markName: string, options: ToggleMarkCascadeOptions = {}): Command =>
  ({ state, chain, editor }) => {
    const {
      negationAttrs = { value: '0' },
      isNegation = (attrs: Record<string, unknown>): boolean => attrs?.value === '0',
 ...
 * @returns {boolean}
 */
export function defaultStyleDetector({
  state,
  selectionMarks,
  markName,
  editor,
 ...
 */
export function getEffectiveStyleId(state: EditorState, selectionMarks: Mark[]): string | null {
 ...
 * @returns {string|null}
 */
export function getStyleIdFromMarks(marks: readonly Mark[]): string | null {
...
export function isStyleTokenEnabled(val: unknown): boolean {
