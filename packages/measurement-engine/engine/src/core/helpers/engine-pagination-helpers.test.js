...
import { generatePageBreaks, ENGINE_PAGINATION_INTERNALS } from './engine-pagination-helpers.js';
...
describe('engine pagination helpers', () => {
  ...
  it('creates header/footer area descriptors', () => {
    ...
    expect(areas.header.metrics.effectiveHeightPx).toBeGreaterThan(0);
  ...
  it('falls back when locating exact break position fails', () => {
    ...
    expect(result).toEqual({ fittedTop: 210, fittedBottom: 220, pos: 20 });
  });
