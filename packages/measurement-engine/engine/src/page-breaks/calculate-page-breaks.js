...
export function calculatePageBreaks(measurementEditor, params = {}) {
  ...
  while (anchorPos < docSize) {
    ...
    // Use reservedHeightPx instead of effectiveHeightPx if available, as it includes offset spacing
    const headerReservedHeightPx = Number.isFinite(headerFooterSections?.header?.reservedHeightPx)
      ? headerFooterSections.header.reservedHeightPx
      : headerEffectiveHeightPx;
    const footerReservedHeightPx = Number.isFinite(headerFooterSections?.footer?.reservedHeightPx)
      ? headerFooterSections.footer.reservedHeightPx
      : footerEffectiveHeightPx;

    // IMPORTANT: Always enforce minimum margins to prevent content from extending to page edges
    // This reserves space for headers/footers even when they have no content
    const MINIMUM_TOP_MARGIN_PX = 48; // 0.5 inch minimum
    const MINIMUM_BOTTOM_MARGIN_PX = 48; // 0.5 inch minimum

    const resolvedMargins = {
      ...baseMargins,
      top: Math.max(baseMargins.top, headerReservedHeightPx, MINIMUM_TOP_MARGIN_PX),
      bottom: Math.max(baseMargins.bottom, footerReservedHeightPx, MINIMUM_BOTTOM_MARGIN_PX),
    };
    ...
    const measuredTop = Number.isFinite(boundary.pageTop) ? boundary.pageTop : null;
    const measuredBottom = Number.isFinite(boundary.pageBottom) ? boundary.pageBottom : null;
    const measuredHeight = Number.isFinite(boundary.usableHeightPx)
      ? boundary.usableHeightPx
      : Number.isFinite(boundary.contentHeightPx)
        ? boundary.contentHeightPx
        : measuredTop != null && measuredBottom != null
          ? Math.max(0, measuredBottom - measuredTop)
          : null;

    let pageTopAbs;
    if (Number.isFinite(anchorCoords?.top) && measuredTop != null) {
      pageTopAbs = Math.max(anchorCoords.top, measuredTop);
    } else if (Number.isFinite(anchorCoords?.top)) {
      pageTopAbs = anchorCoords.top;
    } else if (measuredTop != null) {
      pageTopAbs = measuredTop;
    } else {
      pageTopAbs = 0;
    }
    ...
    if (measuredBottom != null) {
      pageBottomAbs = Math.max(measuredBottom, pageTopAbs + Math.max(0, measuredHeight ?? 0));
    } else if (measuredHeight != null) {
      pageBottomAbs = pageTopAbs + Math.max(0, measuredHeight);
    } else {
      pageBottomAbs = pageTopAbs;
    }
