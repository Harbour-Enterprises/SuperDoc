...
function extractTextFromBlock(block: unknown): string[] {
  const blockObj = block as Record<string, unknown> | null | undefined;
  ...
  if (Array.isArray(blockObj?.content)) {
    return blockObj.content
      .map((part: unknown) =>
        typeof part === 'string' ? part : String((part as Record<string, unknown> | null)?.text || ''),
      )
      .filter(Boolean);
  }
...
function defaultParseCompletion(payload: unknown): string {
  ...
  const choices = payloadObj.choices;
  if (Array.isArray(choices) && choices.length > 0) {
    const choice = choices[0] as Record<string, unknown> | null | undefined;
...
function parseOpenAIStreamChunk(payload: unknown): string | undefined {
  ...
  const payloadObj = payload as Record<string, unknown>;
  if ('choices' in payloadObj && Array.isArray(payloadObj.choices)) {
    return payloadObj.choices
      .map((choice: unknown) => {
        const choiceObj = choice as Record<string, unknown> | null | undefined;
        const delta = choiceObj?.delta as Record<string, unknown> | null | undefined;
...
function parseAnthropicCompletion(payload: unknown): string {
  ...
  return payloadObj.content.flatMap((block: unknown) => extractTextFromBlock(block)).join('');
}
...
function parseAnthropicStreamChunk(payload: unknown): string | undefined {
  ...
  const payloadObj = payload as Record<string, unknown>;
  const delta = payloadObj.delta as Record<string, unknown> | null | undefined;

  if ((payloadObj.type === 'content_block_delta' || payloadObj.type === 'message_delta') && delta?.text) {
    return typeof delta.text === 'string' ? delta.text : undefined;
  }

  if (payloadObj.type === 'message_start' && payloadObj.message) {
    return parseAnthropicCompletion(payloadObj.message);
  }
 ...
 */
function cleanUndefined(object: Record<string, unknown>) {
