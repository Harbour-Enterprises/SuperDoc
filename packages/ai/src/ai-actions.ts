...
export class AIActions {
  ...
  private async executeActionWithCallbacks<T extends Result>(fn: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      // Note: onStreamingEnd is designed for string results from streaming operations,
      // not for Result objects from actions. Actions complete without a streaming callback.
