...
/**
 * Generate OpenAI-compatible tool definitions from SuperDoc editor.
 *
 ...
 * from the editor. Use `await openaiTools(editor)`.
 *
 * @param editor - SuperDoc editor instance
 * @param options - Tool definition options
 * @returns Promise resolving to array of OpenAI tool definitions
 *
 * @example
 * ```typescript
 * import { openaiTools } from '@superdoc-dev/ai/ai-builder/providers';
 * import OpenAI from 'openai';
 *
 * const tools = await openaiTools(editor, {
 *   excludedNodes: ['table'],
 *   excludedMarks: ['trackInsert'],
...
export async function openaiTools(
    ...
    // Define all available tools in OpenAI format with dynamic schema
    // Matching genericTools structure
