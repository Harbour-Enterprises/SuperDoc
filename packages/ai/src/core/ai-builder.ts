...
import {parseJSON, validateInput, getErrorMessage, isPlainObject, isInRange} from '../shared';
import {
...
import {buildAIBuilderSystemPrompt} from '../shared/prompts';
import type {
...
export interface AIBuilderPlan {
    reasoning?: string;
    steps: AIBuilderPlanStep[];
}

export interface AIBuilderConfig {
    // PRIMARY: Pass SuperDoc instance + user config (AIActions created internally)
    superdoc?: SuperDocInstance;
    user?: AIUser;

    // ADVANCED: Pass editor + pre-built AIActions instance
    editor?: Editor;
    aiActions?: AIActions;
...
export interface AIBuilderExecutionResult {
    success: boolean;
...
export class AIBuilder {
    ...
    /**
     * Creates a new AIBuilder instance
     * @param config - Builder configuration
     * @throws {Error} If configuration is invalid
     * 
     * @example
     * ```typescript
     * // Recommended: Pass SuperDoc + user config
     ...
     * });
     * 
     * // Advanced: Pass pre-built AIActions
     * const builder = new AIBuilder({
     *   editor: editor,
     *   aiActions: aiInstance,
     *   provider: { type: 'openai', apiKey: '...', model: 'gpt-4o' }
     * });
     * ```
     */
    constructor(config: AIBuilderConfig) {
        if (!config || !isPlainObject(config)) {
            throw new Error('AIBuilder requires a valid configuration object');
        }
        ...
        if (!config.superdoc && !config.editor) {
            throw new Error(ERROR_MESSAGES.NO_SUPERDOC_OR_EDITOR);
        }
        
        if (config.superdoc && !config.user && !config.aiActions) {
        ...
        this.documentContextProvider =
            config.documentContextProvider ?? (() => this.readDocumentContextFromEditor());
     ...
     * @example
     * ```typescript
     * const result = await builder.execute('Fix grammar issues in the selected text');
     * if (result.success) {
     ...
     * ```
     */
    async execute(prompt: string, options?: CompletionOptions): Promise<AIBuilderExecutionResult> {
    ...
    private async buildPlan(prompt: string, options?: CompletionOptions): Promise<BuilderPlanResult> {
        ...
        const parsed = parseJSON<AIBuilderPlan>(
            planPayload,
            {reasoning: '', steps: []},
            this.enableLogging
        );
    ...
    private extractPlanPayload(response: string): string {
        ...
        const parsed = parseJSON<ParsedResponse>(response, undefined as any, this.enableLogging);
    ...
    private buildSystemPrompt(): string {
        ...
        return buildAIBuilderSystemPrompt(toolDescriptions);
    }
