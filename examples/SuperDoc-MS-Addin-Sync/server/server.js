...
// Optional: Custom middleware to extract user info from token
const extractUserInfo = (req, res, next) => {
  if (req.auth) {
    // The JWT payload is available in req.auth
    req.userInfo = {
      sub: req.auth.sub,
      email: req.auth.email || req.auth["https://example.com/email"], // Handle custom claims
      email_verified: req.auth.email_verified || req.auth["https://example.com/email_verified"],
    };
  }
  next();
};
